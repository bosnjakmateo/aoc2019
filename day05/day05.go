...
func RunDiagnostics(integers []int, input int) {
	increment := 0
...
func opcode4(integers []int, i, mode int) {
	if mode == 0 {
		println(integers[integers[i+1]])
	} else {
		println(integers[i+1])
	}
}
